...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action==null || "".equals(action)) {
			...
			RequestDispatcher dispatcher=request.getRequestDispatcher(address);
			dispatcher.forward(request, response);
		}else if(userBean.getUser().getPermissions().contains("c")){
		   ...
		   //check if file exists and if dest exists
		   ...
		   if(!destFile.exists() || !Security.filterFilename(filename)) {
			   response.setStatus(404);
			   return;
		   }
	       ...
	       HistoryRecordDAO.insert(new HistoryRecord(0, timestamp, userBean.getUser().getUsername(), "upload" , filename));
		}else {
			response.setStatus(404);
			return;
		}
